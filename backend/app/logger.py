...
def log_request_success(ip: str, email: str) -> None:
    ...
    """
    logger.info("Successful contact form submission delivered from IP: {} | Email: {}", ip, _mask_sensitive_data(email))
...
def log_validation_error(path: str, errors: List[Any]) -> None:
    ...
    """
    logger.warning("Validation error on request path {}: {}", path, _mask_sensitive_data(str(errors)))
...
def log_recaptcha_failure(ip: str, reason: str) -> None:
    ...
    """
    logger.warning("Google reCAPTCHA verification rejected for IP {}: {}", ip, _mask_sensitive_data(reason))
...
def log_email_failure(email: str, error: str) -> None:
    ...
    """
    logger.error("Resend API failed to deliver contact email for {}: {}", _mask_sensitive_data(email), _mask_sensitive_data(error))
...
def log_unexpected_exception(exc: Exception) -> None:
    ...
    """
    logger.exception("Unexpected exception caught by global handler: {}", _mask_sensitive_data(str(exc)))
