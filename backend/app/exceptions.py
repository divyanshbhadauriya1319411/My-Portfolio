...
async def validation_exception_handler(
    ...
    errors = exc.errors()
    logger.warning(
        "Validation failed on {}: {} | Status code: 422", request.url.path, errors
    )
...
async def recaptcha_exception_handler(
    request: Request, exc: RecaptchaVerificationError
) -> JSONResponse:
    """
...
async def email_send_exception_handler(
    request: Request, exc: EmailSendError
) -> JSONResponse:
    """
    ...
    return JSONResponse(
        ...
        content={"success": False, "message": "Unable to send email"},
    )
...
async def rate_limit_exception_handler(
    ...
    logger.warning(
        "Rate limit exceeded for IP {} on {} | Status code: 429",
        client_ip,
        request.url.path,
    )
    return JSONResponse(
...
async def general_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    logger.exception(
        "Unexpected exception caught by global handler: {} | Status code: 500 | Stack trace:",
        exc,
    )
